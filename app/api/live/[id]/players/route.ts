...
import { addPlayer, updatePlayer, removePlayer, rejoinPlayer, LiveSessionError } from '@/lib/live-session/actions'
...
export async function PATCH(req: Request, { params }: Ctx) {
  if (!checkAdmin(req)) return NextResponse.json({ error: 'Unauthorised' }, { status: 401 })
  const { id } = await params
  const { player_id, name, level, rejoin } = await req.json()
  if (!player_id) return NextResponse.json({ error: 'player_id required' }, { status: 400 })
  if (rejoin === true) {
    try { await rejoinPlayer(id, player_id); return NextResponse.json({ ok: true }) }
    catch (e) { return fail(e) }
  }
